...
//import links from "./emojis";

async function loadMainEmojis() {
  const url = 'https://sculksmp.github.io/dismoji-db/dismoji-main-emojis.json';
  try {
...
async function initialize() {
  try {
    const emojilinks = await loadMainEmojis();
    loadLinks(emojilinks);
  } catch (error) {
    ...
    if (!link.isTitle) {
        ...
        fav.innerHTML = `
            <svg width="100" height="100" viewbox="0 0 24 24">
                <polygon id="star" points="12,0 15.09,7.24 23.14,8.62 17,14.11 18.18,22.08 12,18.22 5.82,22.08 7,14.11 0.86,8.62 8.91,7.24"/>
            </svg>
        `
